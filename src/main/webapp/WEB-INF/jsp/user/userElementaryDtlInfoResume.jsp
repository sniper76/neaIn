...
<script src="${pageContext.request.contextPath}/js/pageLib/user/userElementaryDtlInfoResume.js"></script>

<script>
$(function() {
	common.invoker.invoke("userElementaryDtlInfoResume");
});
</script>

<form id="userElementaryDtlInfoResume" name="userElementaryDtlInfoResume">
...
<input type="hidden" id="resumeSeq" name="resumeSeq" value="<c:out value="${rtnBaseData.resumeSeq}" />" />
	...
	<div class="bbs_basic">
		<table class="bbs_table write margin_b_20">
			...
			<tr>
				...
				<td colspan="3"><c:out value="${rtnBaseData.addrFullNm}" /></td>
			</tr>
			<tr>
			...
			<tr>
				...
				<td colspan="3">
					<c:out value="${rtnBaseData.disablOptionNm}" />
					<c:forEach items="${rtnDisablData }" var="rtnDisablData" varStatus="stat">
						<c:choose>
							<c:when test="${stat.count eq 1 }">
								<c:out value="${rtnDisablData.disableCdNm}" />
							</c:when>
							<c:otherwise>
								, <c:out value="${rtnDisablData.disableCdNm}" />
							</c:otherwise>
	...
	<div class="bbs_basic">
		<table class="bbs_table write margin_b_20">
			...
			<tr>
				...
				<td><c:out value="${rtnGenrlEduData.eduDegreeCdNm}" /></td>
	...
	<div class="bbs_basic">
		...
		<tr>
			<th scope="row"><c:out value="${rtnOtrTrngList.trnngEduDegreeCd}" /></th>
			<td><c:out value="${rtnOtrTrngList.iscedCd}" /></td>
			...
			<c:choose>
				...
				<c:when test="${not empty rtnLangData}">
					<c:forEach items="${rtnLangData}" var="rtnlanglist">
						...
						<c:choose>
							<c:when test="${rtnlanglist.langTypeCd eq 'LTC0000000001'}" >
								<tr>
									<th scope="row"><span class="exactly y">necessary</span> <spring:message code="field.dic.lang.native" /></th>
									<td colspan="3"><c:out value="${rtnlanglist.langNm}" /></td>
								</tr>
							</c:when>
							<c:when test="${rtnlanglist.langTypeCd eq 'LTC0000000002'}" >
								<tr>
									<th scope="row"><span class="exactly y">necessary</span> <spring:message code="field.dic.foreignlang.name" /></th>
	...
	<div class="bbs_basic">
		<table class="bbs_table write margin_b_20">
			...
			<tr>
				...
				<td><c:out value="${rtnWorkExplist.iscoCd}" /></td>
	...
	<div class="bbs_basic">
		<table class="bbs_table write margin_b_20">
			...
			<tbody>
			<c:choose>
				<c:when test="${empty skillCdList}">
					<tr>
						<th scope="row"><span class="exactly y">necessary</span> </th>
						<td></td>
					</tr>
				</c:when>
				<c:when test="${not empty skillCdList}">
					<c:forEach items="${skillCdList}" var="skillCd">
						<tr>
							<th scope="row"><span class="exactly y">necessary</span> <c:out value="${skillCd.cdEnNm }"/></th>
							<td>
								<c:forEach items="${skillCd.list }" var="cd" varStatus="stat">
									<c:choose>
										<c:when test="${stat.count eq 1}">
											<c:out value="${cd.cdEnNm }" escapeXml="false"/>
										</c:when>
										<c:otherwise>
											, <c:out value="${cd.cdEnNm }" escapeXml="false"/>
										</c:otherwise>
	...
	<div class="bbs_btn_wrap clearfix">
		...
		<span class="bbs_right">
			<button type="button" id="btnUserResumeInfoModify" class="bbs_btn modify" onclick="fnUserResumeInfoModify();"><spring:message code="button.modify" /></button>
		</span>
	</div>
	<!-- //bbs_btn_wrap -->
