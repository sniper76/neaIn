...
//			if ( $('#fairSeq').val() != null && $('#fairSeq').val() != '') {
				...
				that$.dataTables = $('#compnyJoinMngTable').DataTable({
					pageLength: 10,
				    ...
				    serverMethod: 'post',
				    ajax: {
				    	url: contextPath + '/internal/fair/selectCompnyJoinMngListAjax.do?orderColumns='+orderColumns
				    	//, traditional: true
				    	, data: function ( d ) {
				    		return $.extend( {}, d, {
				        		fairSeq: $('#fairSeq').val(),
...
//		                },
				    	{ targets: 2, render: function ( data, type, full, meta ) {
		        				...
		        				return "<a href=\"javascript:void(0);\" onclick=\"jobFairMng.fn.('" + full.compnySeq + "');\">" + full.compnyNm + "</font>" ;
	                    	}
		                },
...
//			}
		...
		fn: {
			...
			updateAction : function( params ) {
				...
				var options = {
				    method: 'post',
				    url: contextPath + '/internal/fair/updateCompnyJoinMngAjax.do',
				    data: params.data,
			...
			pageActionPartici : function(obj) {
				$("#frmCompnyJoinMng").attr("method", "post");
				$("#frmCompnyJoinMng").attr("action", contextPath + "/internal/fair/compnyJoinMng.do");
				$("#frmCompnyJoinMng").submit();
			},


			pageActionRequest : function(obj) {
				$("#frmCompnyJoinMng").attr("method", "post");
				$("#frmCompnyJoinMng").attr("action", contextPath + "/internal/fair/compnyRequestMng.do");
				$("#frmCompnyJoinMng").submit();
			},
			...
			pageActionInvite : function(obj) {
				$("#frmCompnyJoinMng").attr("method", "post");
				$("#frmCompnyJoinMng").attr("action", contextPath + "/internal/fair/compnyInviteMng.do");
				$("#frmCompnyJoinMng").submit();
			}
		}
}
