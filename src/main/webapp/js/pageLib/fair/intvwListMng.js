var	intvwListMng = {
		...
		beforeBind: function() {
			...
			var commCdParams = {
					data: {
						commGrpCdList: ['userAuthCd', 'employFormCd', 'preferntCondCd', 'intvwTypeCd'],
						commGrpUseYn: 'Y'
					}
				};
			...
			$.when( that$.fn.getCommCd(commCdParams), that$.fn.getJcCd() ).done(function( commCd, jcCd ) {				
				...
				that$.commCode['intvwTypeCd'] = groupByCommCd[ that$.fn.toCamelOrSnake('intvwTypeCd', 'snake') ];
		...
		bind: function() {
			...
			if ( $('#fairSeq').val() != null && $('#fairSeq').val() != '') {
				var setColumns = [
					...
					{"data": "userCell"},						//휴대폰
					{"data": "intvwDt"},						//면접일시
					{"data": "intvwTypeCd"},					//면접방법							
					{"data": "intvwAgreeYn"},					//면접수락여부
					{"data": "intvwOpenDt"},					//열람일시
					{"data": "referralYn"},						//알선여부
					
					{"data": "fairSeq"},						//채용행사시퀀스
					{"data": "applicSeq"},						//입사지원시퀀스
					{"data": "vacancySeq"},						//채용공고시퀀스
					{"data": "userSeq"},						//회원시퀀스
					];
				
				var orderColumns = setColumns.map(function(x) { return x.data });
				
				that$.dataTables = $('#intvwListMngTable').DataTable({
					pageLength: 10,
				    ...
				    serverMethod: 'post',
				    ajax: {
				    	url: contextPath + '/internal/fair/selectIntvwMngListAjax.do?orderColumns='+orderColumns
				    	, data: function ( d ) {
				        	...
				        	return $.extend( {}, d, {
				        		...
				        		intvwAgreeYn: $('input[name=intvwAgreeYn]:checked').val(),
				    ...
				    columnDefs: [
	                    ...
	                    { targets: 4, render: function ( data, type, full, meta ) {
		        				return "<a href=\"javascript:void(0);\" onclick=\"intvwListMng.fn.popupVccyAction('" + full.vacancySeq + "');\">" + data + "</font>" ;
	                    	}
		                },
		                ...
		                { targets: 8, render: function ( data, type, full, meta ) {
		        				return "<a href=\"javascript:void(0);\" onclick=\"intvwListMng.fn.popupUsrAction('" + full.userSeq + "');\">" + data + "</font>" ;
	                    	}
		                },
		                ...
		                { targets: 12, render: function ( data, type, full, meta ) {
	                			if (data != null && data != '') {
	    	                		var obj = that$.commCode['intvwTypeCd'].find(function(x) { return x.dtlCd == data });
	    	                		return that$.fn.commCdLang(obj) ;                    	
		                		} else {
		                			return '';
		                		}
		                ...
		                { targets: [16, 17, 18, 19], visible: false, searchable: false },
				    ],
				    select: { 
				    	style: 'multi' 
				    },
						...
						alertify.confirm( t( 'cpes.save.btn.confirm.msg' ), function (ex) {
							if (ex) {
								...
								$.when( that$.fn.insertAction( params ) ).done(function( result ) {
									alertify.alert( t( 'cpes.reg.success.msg' ), function (e) {
										if(e) {
		...
		fn: {
			...
			popupUsrAction : function(userSeq) {
				alert(userSeq + ' 회원 상세 팝업');
			},
