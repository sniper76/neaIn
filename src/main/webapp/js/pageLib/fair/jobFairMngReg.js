...
//			$('#jcCd').attr('required', true);
		...
		afterBind: function() {
			...
			$('#btnSubmit').on('click', function(e) {
				//that$.fn.insertAction();
            });
			
			$("#frmJobFairMngReg").submit(function(e) {
			    var form$ = $(this);
				e.preventDefault();
			    ...
			    var thumbFileGrpSeq = DEXT5UPLOAD.GetTotalFileCount("thumbCommonUploadFair");
			    var fairFileGrpSeq  = DEXT5UPLOAD.GetTotalFileCount("fairCommonUploadFair");
			    
			    if (parseInt(thumbFileGrpSeq) < 1 ) {
			    ...
			    if ($("#fairDtlExplnKh").val() == "") {
		    		alertify.alert(explnBlank, function(e) {
						$("#fairDtlExplnKh").focus();
					});
					return;
			    }
		...
		fn: {	
			insertAction : function(params) {
				...
				var options = {
					    method: 'post',
					    url: contextPath + '/internal/fair/insertJobFairMngAjax.do',
					    data: params.data,
					    dataType: 'json',
					};
...
function fnFileFormSave() {
	var params = {
			data: $("#frmJobFairMngReg").serializeObject()
		};
