...
function dext5uploadEvnt(){
	...
	var teachFileGrpSeq = $('#teachFileGrpSeq').val();
...
function updateWorkshopSave(){
	...
	alertify.confirm("워크샵을 수정하시겠습니까?", function(e){
		if(e){
...
function fnFileFormSave(){
	...
	$.ajax({
		type: 'post',
		url: contextPath + "/internal/workshop/updateWorkshopAjax.do",
		dataType: 'json',
		data: formData,
		contentType: false,
		processData: false,
		success: function(data) {
			if(data.result > 0){
				alertify.alert("워크샵 수정이 완료되었습니다.", function(){
					location.href = contextPath + "/internal/workshop/list.do";
				});
			}else{
...
function deleteWorkshopSave(){
	var message = "";
	if($('#patcptnCnt').val() > 0){
		message = "신청자가 있는 워크샵입니다. 삭제 시에는 신청자 정보까지 삭제됩니다. 그래도 삭제하시겠습니까?";
	}else{
		message = "워크샵을 삭제하시겠습니까?";
	}
	...
	alertify.confirm(message, function(e){
		if(e){
			$.ajax({
				type: 'post',
				url: contextPath + "/internal/workshop/deleteWorkshopAjax.do",
				dataType: 'json',
				data: {
				...
				success: function(data) {
					if(data.result > 0){
						alertify.alert("워크샵 삭제가 완료되었습니다.", function(){
							location.href = contextPath + "/internal/workshop/list.do";
						});
					}else{
						alertify.alert(resumeInsertErrorMsg);
