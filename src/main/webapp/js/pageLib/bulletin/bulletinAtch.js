var	bulletinAtchList = {
    pageYn : 'N',
    name: 'bulletinAtchList',
    dataTables: null,
    getRowData:[],
    ...
    bind: function() {
        ...
        // init dataTable
        that$.dataTables = $('#bulletinAtch').DataTable({
            pageLength: 10,
            ...
            stateSaveCallback: function(settings,data) {
                localStorage.setItem( bulletinAtchList.name + settings.sInstance, JSON.stringify(data) );
              },
            stateLoadCallback: function(settings) {
                if (bulletinAtchList.pageYn == 'Y') {
                    return JSON.parse( localStorage.getItem( bulletinAtchList.name + settings.sInstance ) );
                } else {
                    localStorage.clear();
                    return JSON.parse( localStorage.getItem( bulletinAtchList.name + settings.sInstance ) );
                }
...
function insertBulletin(){
    ...
    sessionStorage.setItem(bulletinAtchList.name + "." + bulletinAtchList.method, JSON.stringify(sessData));
    $("#frm").attr("method", "post");
    $("#frm").attr("action", contextPath + "/internal/bulletin/bulletinAtchReg.do?pg="+$("#programId").val());
    $("#frm").submit();
...
function pageAction(bulletinSeq){
    ...
    $("#frm").attr("method", "post");
    $("#frm").attr("action", contextPath + "/internal/bulletin/bulletinAtchDtl.do?pg="+$("#programId").val());
    $("#frm").submit();
}
