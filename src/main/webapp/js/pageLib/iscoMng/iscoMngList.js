var	iscoMngList = {
		...
		bind: function() {
			...
			that$.dataTables = $('#iscoMngTable').DataTable({
				pageLength: 10,
			    ...
			    serverMethod: 'post',
			    ajax: {
			    	url: contextPath + '/internal/iscoMng/selectIscoMngListAjax.do?orderColumns='+orderColumns
			    	, data: function ( d ) {
			    ...
			    columnDefs: [
                    { targets: 4, render: function ( data, type, full, meta ) {
	        				...
	        				return "<a href=\"javascript:void(0);\" onclick=\"iscoMngList.fn.pageAction('" + full.iscoCd + "');\">" + full.depthNm4 + "</font>" ;
                    	}
	                },
			    ],
			    order: [[0, 'asc']],
		...
		fn: {
			...
			pageAction : function(iscoCd) {
				$("#frm").find("#detailIscoCd").val(iscoCd);

				$("#frm").attr("method", "post");
				$("#frm").attr("action", contextPath + "/internal/iscoMng/detail.do");
				$("#frm").submit();
			},
