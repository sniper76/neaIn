	...
	var	userCmpnyDtlInfoAplcList = {
			pageYn : 'N',
			name: 'userCmpnyDtlInfoAplcList',
			dataTables: null,
			...
			beforeBind: function() {
				...
				if (sessData) {
					userCmpnyDtlInfoAplcList.pageYn = 'Y';
			...
			bind: function() {
				...
				that$.dataTables = $('#myTable').DataTable({
			        ...
			        stateSaveCallback: function(settings,data) {
			            localStorage.setItem( userCmpnyDtlInfoAplcList.name + settings.sInstance, JSON.stringify(data) );
			          },
			        stateLoadCallback: function(settings) {
			        	if (userCmpnyDtlInfoAplcList.pageYn == 'Y') {
			        		return JSON.parse( localStorage.getItem( userCmpnyDtlInfoAplcList.name + settings.sInstance ) );
			        	} else {
			        		localStorage.clear();
			        		return JSON.parse( localStorage.getItem( userCmpnyDtlInfoAplcList.name + settings.sInstance ) );
			        	}
			        ...
			        serverMethod: 'post',
			        ajax: {
			            url: contextPath +"/internal/user/selectUserCmpnyDtlInfoAplcListSubAjax.do?orderColumns="+orderColumns
			            , dataType: 'json'
			            //, data: param
			            , data: function ( d ) {
							return $.extend( {}, d, {
