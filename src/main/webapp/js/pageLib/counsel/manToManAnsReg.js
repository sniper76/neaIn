...
var	manToManAnsReg = {
		...
		afterBind: function() {
            ...
            $('#btnDelete').on('click', function(e) {
            	// confirm dialog
            	alertify.confirm("삭제 하시겠습니까?", function (ex) {
            	    if (ex) {
		...
		fn: {
			deleteAction : function(e) {
				...
				.done(function (res, status, responseObj) {
					if ( res.result.counselSeq ) {
						...
						window.location.href = contextPath + '/internal/counsel/manToMan.do?method=' + method;
					} else {
			...
			updateAction : function(e) {
				$("#frmManToManAnsReg").submit(function(e) {
				    var form$ = $(this);
					var param = form$.serializeObject();
...
function fnFileFormSave() {
	var param = $('#frmManToManAnsReg').serializeObject();

	$('#frmManToManAnsReg').find("#btnDelete").attr('disabled', true);
	$('#frmManToManAnsReg').find("#btnSubmit").attr('disabled', true);
	...
	.done(function (res, status, responseObj) {
		if ( res.result.counselSeq ) {
			...
			alertify.alert("인증 안됨", function (e) {
				if(e) {
					$('#frmManToManAnsReg').find("#btnDelete").attr('disabled', false);
					$('#frmManToManAnsReg').find("#btnSubmit").attr('disabled', false);
				}
			});
		}
	}).fail(function (result, status, responseObj) {
