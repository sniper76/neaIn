...
public class UploadHelper {
	...
	public void upload() {
		...
		if (!(request instanceof MultipartHttpServletRequest)) {
			throw new IllegalArgumentException("please 'multipart/form-data' enc-type for your form.");
		}
	 ...
	 */
	public JSONArray getFilesToJSONArray(MultipartHttpServletRequest multipartRequest) throws IOException {
		JSONArray jArray = new JSONArray();
		...
		while (iter.hasNext()) {
			...
			for (int i = 0; i < getFileList.size(); i++) {
				...
				file.setSaveNm((KeyGenerateUtil.getUUID()).replaceAll("-", "") + "." + FileUtil.getFileType(mpf.getOriginalFilename()));
	...
	private void parseMultipart(MultipartHttpServletRequest multipartRequest) throws IOException {
		...
		while (iter.hasNext()) {
			...
			for (int i = 0; i < getFileList.size(); i++) {
				...
				file.setOriginalNm(mpf.getOriginalFilename());
				file.setSaveNm((KeyGenerateUtil.getUUID()).replaceAll("-", "") + "."
						+ FileUtil.getFileType(mpf.getOriginalFilename()));
				file.setFileSize(String.valueOf(mpf.getSize()));
	...
	private String makeFileUrl(String bizName, String subName) {
		File fDir = new File(BaseConfig.getValue("fileUploadDir") + BaseConfig.getValue("fileUploadUrl")
				+ BaseConfig.getValue("SEPARATOR") + bizName);
		if (!fDir.exists()) {
			fDir.mkdirs();
		}
	...
	private void write(MultipartFile mfile, FileBean file) throws IOException {
		File destFile = new File(this.baseDir + file.getSaveFilePath(), file.getSaveNm());
